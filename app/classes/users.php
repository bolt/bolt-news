...
class Users {
    ...
    public function deleteUser($id) {
        ...
        if (empty($user['id'])) {
            $this->session->getFlashBag()->add('error', 'That user does not exist.');
            return false;
        } else {
    ...
    public function login($user, $password) {
        ...
        if (empty($user)) {
            $this->session->getFlashBag()->add('error', 'Username or password not correct. Please check your input.');
            return false;
        }

	echo "cwd: ". getcwd() . "<br />\n";
	echo "dir: " . __DIR__."/phpass/PasswordHash.php" . "<br />\n";
        require_once("app/classes/phpass/PasswordHash.php");
        $hasher = new PasswordHash(8, TRUE);

        if ($hasher->CheckPassword($password, $user['password'])) {

            if (!$user['enabled']) {
                $this->session->getFlashBag()->add('error', 'Your account is disabled. Sorry about that.');
                return false;
            }
            ...
            $this->session->getFlashBag()->add('success', "You've been logged on successfully.");
